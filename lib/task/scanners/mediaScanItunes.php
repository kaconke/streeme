...
while( $value = $itunes_parser->getTrack() )
{
  ...
  //decode the itunes file scheme 
  $value[ 'Location' ] = StreemeUtil::itunes_format_decode( $value[ 'Location' ], StreemeUtil::is_windows(), $mapped_drive_locations);
  
  //convert it from user's filesystem value to UTF-8
  $value[ 'Location' ] = iconv( sfConfig::get( app_filesystem_encoding, 'ISO-8859-1' ), 'UTF-8//TRANSLIT', $value[ 'Location' ] );
