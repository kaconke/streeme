CREATE TABLE album (id BIGSERIAL, scan_id BIGINT, name VARCHAR(255) UNIQUE, amazon_flagged SMALLINT DEFAULT 0, meta_flagged SMALLINT DEFAULT 0, folders_flagged SMALLINT DEFAULT 0, service_flagged SMALLINT DEFAULT 0, has_art SMALLINT DEFAULT 0, PRIMARY KEY(id));
...
CREATE TABLE playlist (id BIGSERIAL, name VARCHAR(255), PRIMARY KEY(id));
CREATE TABLE playlist_files (id BIGSERIAL, playlist_id BIGINT DEFAULT 0, filename text, PRIMARY KEY(id));
CREATE TABLE scan (id BIGSERIAL, scan_time VARCHAR(255), scan_type VARCHAR(255), PRIMARY KEY(id));
CREATE TABLE song (id BIGSERIAL, unique_id VARCHAR(255), artist_id BIGINT DEFAULT 0, album_id BIGINT DEFAULT 0, genre_id BIGINT DEFAULT 0, last_scan_id BIGINT DEFAULT 0, name VARCHAR(255), length VARCHAR(255), accurate_length BIGINT, filesize BIGINT, bitrate BIGINT, yearpublished BIGINT, tracknumber BIGINT, label VARCHAR(255), mtime BIGINT, atime BIGINT, filename text, PRIMARY KEY(id));
...
CREATE INDEX sort_index ON song (artist_id, album_id, tracknumber, mtime);
...
CREATE INDEX last_scan_id ON song (last_scan_id);
CREATE INDEX genres_index ON song (genre_id);
