...
/*
 * This file is a mockup of the sfWebBrowserPlugin package for unit and functional testing Streeme/Echonest API
 ...
 * @version    0.9
 */
class sfWebBrowserMock extends sfWebBrowser
{
  protected
  ...
  {
    $filename = md5(sprintf('%s-%s-%s-%s', $uri, $method, implode(',',$parameters), implode(',',$headers))).'.txt';
    $directory = dirname(__FILE__) . '/../fixtures/sfWebBrowserMock/';
    $rawHeaders = fopen($directory . 'echonestHeaders.txt', 'r');
    $responseHeaders = array();
    while($row = fgets($rawHeaders))
    {
      $tmp = explode(':', $row);
      $responseHeaders[$tmp[0]] = str_replace(array("\r","\n"), '', $tmp[1]);
    }

    if($result = file_get_contents($directory.$filename))
    {
    ...
    else
    {
      ...
      $this->setResponseHeaders($responseHeaders);
      $this->setResponseText('404 - not found.');
    }
    return $this;
  }
