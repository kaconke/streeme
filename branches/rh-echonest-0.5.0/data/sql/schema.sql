...
CREATE TABLE echonest_properties (id INTEGER PRIMARY KEY AUTOINCREMENT, en_item_id VARCHAR(255), en_version VARCHAR(255), en_date_added VARCHAR(255), en_artist_id VARCHAR(255), en_song_id VARCHAR(255), en_foreign_id VARCHAR(255), en_audio_md5 VARCHAR(255), en_location VARCHAR(255), en_mode INTEGER, en_time_signature INTEGER, en_key INTEGER, en_duration DOUBLE, en_loudness DOUBLE, en_energy DOUBLE, en_tempo DOUBLE, en_danceability DOUBLE, en_song_hotttnesss DOUBLE, en_artist_hotttnesss DOUBLE, en_artist_familiarity DOUBLE, en_latitude DOUBLE, en_longitude DOUBLE);
...
CREATE TABLE song (id INTEGER PRIMARY KEY AUTOINCREMENT, scan_id INTEGER DEFAULT 0, unique_id VARCHAR(255), artist_id INTEGER DEFAULT 0, album_id INTEGER DEFAULT 0, name VARCHAR(255), length VARCHAR(255), accurate_length INTEGER DEFAULT 0, filesize INTEGER DEFAULT 0, bitrate INTEGER DEFAULT 0, yearpublished INTEGER DEFAULT 1969, tracknumber INTEGER DEFAULT 1, label VARCHAR(255), isremix INTEGER DEFAULT 0, echonest_flagged INTEGER DEFAULT 0, mtime INTEGER, atime INTEGER, filename TEXT, set_index INTEGER, set_total INTEGER, comments TEXT);
...
CREATE INDEX en_item_id_index_idx ON echonest_properties (en_item_id);
...
CREATE INDEX set_index_index_idx ON song (set_index);
CREATE INDEX scan_id_idx ON song (scan_id);
CREATE INDEX isremix_index_idx ON song (isremix);
CREATE INDEX echonest_flagged_index_idx ON song (echonest_flagged);
